    ...
    def migrate_hackathons
      ...
      Airtable::Hackathon.all(sort: {created_at: :asc}).each do |record|
        ...
        ActiveRecord::Base.transaction do
          ...
          hackathon.send(:record, :imported_from_airtable, data: record.fields.except(airtable_swag_mailing_address_fields))
          ...
          if record.swag_mailing_address
            ...
            hackathon.swag_mailing_address.send(:record, :imported_from_airtable, data: original_address)
          end
          ...
          hackathon.applicant = User.find_or_initialize_by(email_address: record.applicant_email) do |applicant|
            ...
            applicant.send(:record, :imported_from_airtable)
          end
          ...
          if hackathon.in_person? && !hackathon.geocoded?
            hackathon.send(:record, :airtable_migration_geocoding_error, data: {airtable_coordinates: record.coordinates})
          end
          if record.coordinates && hackathon.geocoded? && hackathon.distance_to(record.coordinates) > 100
            hackathon.send(:record, :airtable_migration_geocoding_distance_error, data: {airtable_coordinates: record.coordinates})
          end
        end
      end
    end
    ...
    def migrate_subscriptions
      ...
      Airtable::Subscriber.all(sort: {created_at: :asc}).each do |record|
        ...
        ActiveRecord::Base.transaction do
          ...
          subscription.send(:record, :imported_from_airtable, data: record.fields)
          ...
          subscription.subscriber = User.find_or_initialize_by(email_address: record.email) do |subscriber|
            ...
            subscriber.send(:record, :imported_from_airtable)
          end
